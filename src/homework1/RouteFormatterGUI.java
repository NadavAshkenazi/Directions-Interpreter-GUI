...
public class RouteFormatterGUI extends JPanel {
	...
	public void addSegment(GeoSegment segment) {
		...
		if (route == null){
			...
			model.add(model.size(), segment);
		}
		RouteFormatter walking = new WalkingRouteFormatter();
		RouteFormatter driving = new DrivingRouteFormatter();

		txtWalkingDirections.setText(null);
		txtDrivingDirections.setText(null);
		txtWalkingDirections.insert(walking.computeDirections(route, 0) ,0);
		txtDrivingDirections.insert(driving.computeDirections(route, 0) ,0);
	}
